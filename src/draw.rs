...
use crate::signal::{self, spectrogram};
use crate::Model;
...
pub fn draw_upper_signal(
    upper: MyDrawingArea,
    m: &Model,
) -> Result<(), Box<dyn std::error::Error>> {
    let max = m.max64[0].0(m, m.range());
    let signal: Vec<(usize, f64)> = m.aiff_data[m.range()]
        .iter()
        .enumerate()
        .map(|(i, f)| (i + m.start, *f as f64 / max))
        .collect();
...
pub fn draw_upper_fft(upper: MyDrawingArea, m: &Model) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let upper = upper.margin(0, 0, 0, 20);
    let caption = format!("{}  {:?}  Max {:.1}", m.aiff_name, m.range(), fmax,);
